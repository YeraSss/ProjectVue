...
export default {
  methods: {
    ...mapActions({
      fetchGroupIndicatorsWithId: "fetchGroupIndicatorsWithId",
    ...
    resetOtherReports(currentReport) {
      this.$store.state.reports.forEach((report) => {
    ...
    toggleReport(report) {
      report.clicked = true;
      this.resetOtherReports(report);
      this.fetchGroupIndicatorsWithId(report.id);
